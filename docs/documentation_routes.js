...
// Branching
router.post('/examples/branching/roro-mode-answer', function (req, res) {
  // Get the answer from session data
  // The name between the quotes is the same as the 'name' attribute on the input elements
  // However in JavaScript we can't use hyphens in variable names

  const roroMode = req.session.data['roro-mode']

  if (roroMode === 'tourist') {
    res.redirect('/search-MVP/voyageBranching/voyage-Tourist-filtered')
  } else {
    res.redirect('/search-MVP/voyageBranching/voyage-Freight-filtered')
  }
})
